...
consolidado_lamba_fit <- readRDS(file.path(proj_path,"data","processed","consolidado_lamba_fit_corpacancha.rds"))
loglambda_p <- readRDS(file.path(proj_path,"data","processed","loglambda_p_corpacancha.rds"))
loglambda <- readRDS(file.path(proj_path,"data","processed","loglambda_corpacancha.rds"))
```
...
intensidades <- c()
for (i in 1:1000) {
  intensidades[i] <- list(ppm(muestras[[i]] ~ s(x,y,k=17),
...
```{r}
cociente <- matrix(NA,nrow = 141,ncol=1000)
for (i in 1:1000) {
...
```{r}
cociente_matrix <- matrix(NA, nrow = 141,ncol = 3)
for (i in 1:141) {
...
```{r}
plot(c(1:141), cociente_promedio,
     ylim = c(0,1), col="green",
     type = "l", ylab = "Cociente")
points(c(1:141), cociente_promedio + qnorm(0.975)*cociente_sd,
     type = "l", col="red")
points(c(1:141), cociente_promedio - qnorm(0.975)*cociente_sd,
       type = "l", col="red")
points(c(1:141), exp(loglambda)/exp(loglambda_p),
       type = "l", col="blue")
grid()
```
...
```{r}
plot(c(1:141), exp(loglambda)/exp(loglambda_p),
     ylim = c(0.1,1.9), col="red", lty=1, pch=1,
     type = "l", ylab = "Cociente",xlab="N° de Observacion")
points(c(1:141), cociente_matrix[,2], lty=3,
     type = "l", col="blue")
points(c(1:141), cociente_matrix[,3], lty=3,
       type = "l", col="blue")
grid()
```

```{r}
montecarlo_cociente <- as.data.frame(cbind(virsel_ppp$x,virsel_ppp$y,
                                           cociente_matrix,
                                           exp(loglambda)/exp(loglambda_p)))
names(montecarlo_cociente) <- c("x","y","quantile50","quantile025","quantuile975","cociente")
#persp(density(virsel_ppp)/density(consolidados_ppp),
#      zlab="", main="",

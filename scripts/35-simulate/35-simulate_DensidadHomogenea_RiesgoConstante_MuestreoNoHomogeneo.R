...
library(splines)
#=#=#=#=#=#=#=#=#=#=#=#=#=#=#=#=#=#=#=#=#=#=#=#=#=#=#=#=#=#=#=#=#=#=#=#
#=#=# Densidad poblacional homogenea, riesgo espacial constante, muestreo aleatorio no homogeneo
h_pi <- 2.5
k_pi <- 7.5
a_pi <- -1/4
b_pi <- -1/16
...
spp_lambda_fit_predict_positvo <- predict(spp_lambda_fit_positvo, type="intensity",
                                          ngrid=100)
spp_lambda_fit_predict_negativo <- predict(spp_lambda_fit_negativo, type="intensity",
                                           ngrid=100)
risk_estimated <- spp_lambda_fit_predict_positvo/(spp_lambda_fit_predict_positvo+
...
plot(risk_estimated)
x11()
plot(spp_lambda_fit_predict_positvo/(alfa*spp_lambda_fit_predict_negativo))

temporal <- spp_lambda_fit_predict_positvo/(alfa*spp_lambda_fit_predict_negativo)
summary(temporal)
...
z <- psych::logistic(predict.gam(theta_fit_gam, newdata=data.frame(x=grid[,1], y=grid[,2])))
x11()
imagen_plot(z)
x11()
persp(matrix(z, 100, 100))
